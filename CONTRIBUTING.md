...
- `tests/` — a full suite written against that spec
- `tensorkit/` — milestones 1-3 implemented; milestones 4-8 are stubs
- CI, tooling, and a milestone roadmap

So the early milestones are done and the later ones are not. Each remaining stub carries the contract it must satisfy; filling those in is the exercise.

**The milestone 1-3 code on `main` was generated by an AI agent**, working from `SPEC.md` and the test suite under exactly the rules below. That is stated here rather than buried, because you are entitled to know which parts of a repository a person wrote. The specification, the test suite and the tooling are the human design work.
...
`reference/m1-3` holds the same milestone 1-3 implementation that is on `main`, kept as a standalone branch so it can be diffed in isolation.

Milestones 1-3 complete: 92/92 tests green.
